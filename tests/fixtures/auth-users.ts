...
export const authUsers = test.extend<AuthUsersFixture>({
  ...
  users: async ({ db, steamIds, browser, baseURL }, use) => {
    ...
    const secret = (await collection.findOne({ name: 'session' })) as {
      name: string
      value: string
    } | null
    ...
    if (secret === null) {
      ...
      await collection.insertOne({ name: 'session', value: value.toString('hex') })
      sessionKey = value
    } else {
      sessionKey = Buffer.from(secret.value, 'hex')
    }
    ...
    await Promise.all(
      steamIds.map(async steamId => {
        ...
        const token = jsonwebtoken.sign({ id: steamId }, sessionKey, {
          expiresIn: '7d',
        })
