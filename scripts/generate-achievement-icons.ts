...
function serializeNode(node: ParsedNode, indent: string): string {
  ...
  for (const [k, v] of Object.entries(el.attrs)) {
    if (k === 'fill') {
      ...
      // Remove colored fills — children inherit "currentColor" from the SVG parent.
      // Preserve fill="none" (transparent intent) and fill="currentColor".
      // Also strip url() gradient references (gradient defs are dropped above).
